...
try:
    ...
    cpu_info = (cpuinfo.get_cpu_info()['brand_raw'].partition("@")[0],psutil.cpu_freq()[0])
    activated = None
   ...
   #API
    ...
    class crucial_data_getter():
        def __init__(self):
            self.x = open("Data/crucial_data.txt","r")
            y = self.x.readlines()
            for i in y:
              if i != "\n":
                collective_data[i.split("]")[0].replace("[","")] = str(i.split("=")[1]).replace("\n","")
              else:
                pass
            
        def update_element(self,token,new_data):
            global collective_data
            self.x.close()
            collective_data[token] = new_data
        
        def update_file(self):
          self.write_file = open("Data/crucial_data.txt","w")
          print(collective_data)
          for i in collective_data:
            self.write_file.write("["+str(i)+"]="+str(collective_data[i])+"\n")    
    ...
    getter = crucial_data_getter()
    try:
        OS_URL = collective_data["Update-Server"]
        ...
        def check_version_diff(versions):
            try:
        ...
        if check_version_diff([current_OS_ver,git_OS_ver])==current_OS_ver:
            Update_required = False
    ...
    def check_activation():
        activation_codes = requests.get(collective_data["Activation-Server"], timeout=10)
        activation_keys = [i.replace("\r","").replace("\n","").replace("<html>","").replace("</html>","") for i in html.unescape(activation_codes.text).split("<br>")]
        return activation_keys
    
    def check_if_activated():
        if WIFI==True:
            if collective_data["Activation-Code"] in check_activation():
                activated = True
                return True
            else:
                activated = False
    ...
    def TEMPLATE_APP_SETTIGNS():
        def open_activation_website():
            webbrowser.open("https://boxos-activations.github.io/activation/")
        ...
        Place_holder_frame3 = Frame(main,height=170,width=350,bg="white",highlightbackground="Black",highlightthickness=4,bd=0).place(x=50,y=240)
        ...
        ACTIVATIONLABEL = Label(main,text="Activate BoxOS: ",font=("default", 15),bg="white").place(x=150,y=245)
        ACTIVATIONLABEL = Label(main,text="Activation: ",font=("default", 12),bg="white").place(x=80,y=335)
        
        ACTIVATIONLABEL_input = Entry(main,font=("default", 10),borderwidth=4,width=27,highlightthickness=0)
        ACTIVATIONLABEL_input.place(x=175,y=335) 
        
        
        act = check_if_activated()
        
        if act == True:
            ACTIVATIONLABEL_WEBSITE = Label(main,text="BoxOS already activated!",fg="#6a9662",font=("default", 10),bg="white",bd=0)
            ACTIVATIONLABEL_WEBSITE.place(x=75,y=270)
            ACTIVATIONLABEL_input.insert(0,str(collective_data["Activation-Code"]))
            ACTIVATIONLABEL_input.config(state="disabled")
            
        else:    
            ACTIVATIONLABEL_WEBSITE = Button(main,text="Go to https://boxos-activations.github.io/activation/ \n for a Activation key!",fg="red",font=("default", 10),bg="white",bd=0,command=open_activation_website)
            ACTIVATIONLABEL_WEBSITE.place(x=75,y=270)
        ...
        def FILE_UPDATE():
            ...
            if ACTIVATIONLABEL_input.get() in check_activation():
                collective_data["Activation-Code"] = ACTIVATIONLABEL_input.get()
                crucial_file = open("Data/crucial_data.txt","w")
                for i in collective_data:
                    crucial_file.write("["+str(i)+"]="+str(collective_data[i])+"\n")
                    
                ACTIVATIONLABEL_WEBSITE.config(text="BoxOS already activated!",fg="lime",font=("default", 10),bg="white",bd=0)
                ACTIVATIONLABEL_input.config(state="disabled")
            ...
            if INP.get() != "":
                ...
                if CMD_text == "help":
                   ...
                   OUTPUT.delete("1.0",END)
                    ...
                    OUTPUT.insert(END,f"Version of BoxOS listed on github servers: {git_OS_ver}")

                elif CMD_text == "BoxOS-activation":
                    OUTPUT.insert(END,f"Activation key used: {gcollective_data['Activation-Code']}") #
    ...
    Button(start_btn_funtc,bg="#222222",text="Shutdown",font=("default 12"),borderwidth=0,activebackground="#272822",bd=0,command=shutdown).place(x=150,y=330)
    
    #TIME
    calender = True
    time_btn_funtc = Frame(tk,width=330,height=350,bg='#222222')
    ff_time = Label(time_btn_funtc,bg='#222222',fg="white",text="0:0:0",font="ariel 35")
    
    Am_Pm = Label(time_btn_funtc,bg='#222222',fg="white",text="N/A",font="ariel 20")

    cal = Calendar(time_btn_funtc, background="#222222", disabledbackground="#222222", bordercolor="#222222", 
                   headersbackground="#222222", normalbackground="#222222", foreground='white', 
                   normalforeground='white', headersforeground='white',weekendbackground="#222222",
                   weekendforeground="white",othermonthbackground="#222222",font="ariel 12",
                   othermonthwebackground="#222222")
    cal.place(x=20,y=100)
    ff_time.place(x=20,y=0)
    Am_Pm.place(x=215,y=20)


    def toggle_2(event):
        global calender
        if calender:
          time_btn_funtc.place(x=x-330,y=y-390)
          calender = False

        else:
          time_btn_funtc.place_forget()
          calender = True
            
    time_label = Label(tk,font='ariel 20',bg='#272822',fg='#FFFFFF',text='')
    time_label.place(x=x-100,y=y-40)
    time_label.bind("<Button-1>",toggle_2)
    time_label.lift()
    ...
    def TIME_GUI():
        ...
        ff_time.config(text=now.strftime('%I:%M:%S').lstrip("0").replace(" 0", " "))
        Am_Pm.config(text=now.strftime("%p"))
    ...
    F12_for_BIOS = Label(Boot_screen,text="F1 for BIOS",bg="#222222",fg="white",font=("Arial", 7, "bold")).place(x=0,y=y-20)
    ...
    Boot_screen.focus_set()
    bar['value'] = 0
    bar.place(relx=0.5,rely=0.8,anchor="center")

    def BIOS(event):
        bios_frame = Frame(tk,width=x,height=y,bg="#030098")
        main_frame = Frame(tk,width=x,height=y-100,bg="#a9a9a9")
        ...
        def Escape(event):
            bios_frame.unbind("<Escape>")
            bios_frame.destroy()
            main_frame.destroy() 
            
            
        Label(bios_frame,text="BIOS SETUP UTILITY",bg="#030098",fg="white",font=('Terminal', 20, 'bold')).place(x=x-1150,y=10)
        
        Label(main_frame,text="PROCESSOR",bg="#a9a9a9",fg="black",font=('Terminal', 20, 'bold')).place(x=5,y=100)
        Label(main_frame,text=str(cpu_info[0]).upper(),bg="#a9a9a9",fg="#5d5d5d",font=('Terminal', 20, 'bold')).place(x=150,y=160)
        Label(main_frame,text="SPEED: "+str(cpu_info[1])+"MHz",bg="#a9a9a9",fg="#5d5d5d",font=('Terminal', 20, 'bold')).place(x=150,y=220)
        Label(main_frame,text="CORES: "+str(multiprocessing.cpu_count()),bg="#a9a9a9",fg="#5d5d5d",font=('Terminal', 20, 'bold')).place(x=150,y=280)
        
        
        Label(main_frame,text="SYSTEM MEMORY",bg="#a9a9a9",fg="black",font=('Terminal', 20, 'bold')).place(x=5,y=360)
        Label(main_frame,text="SIZE: "+str(convert_size((psutil.virtual_memory()[0]))),bg="#a9a9a9",fg="#5d5d5d",font=('Terminal', 20, 'bold')).place(x=150,y=410)
        
        
        Label(main_frame,text="SYSTEM DATE: ["+str(date_time.strftime("%d/%m/%Y"))+"]",bg="#a9a9a9",fg="#030098",font=('Terminal', 20, 'bold')).place(x=5,y=610)
        
        Label(bios_frame,text="[ESC] EXIT",font=('Terminal', 16, 'bold'),bg="#030098",fg="white").place(x=0,y=y-30)
        bios_frame.focus_set()
        bios_frame.bind("<Escape>",Escape)
    
        bios_frame.place(x=0,y=0)
        main_frame.place(anchor="c",relx=.5,rely=.5)

    
    Boot_screen.bind("<F1>",BIOS)
